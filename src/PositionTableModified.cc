...
std::vector<L3Address> PositionTableModified::getAddresses() const
{
...
bool PositionTableModified::hasPosition(const L3Address& address) const
{
    AddressToPositionMap::const_iterator it = addressToPositionMap.find(address);
    return it != addressToPositionMap.end();
}

Coord PositionTableModified::getPosition(const L3Address& address) const
{
    AddressToPositionMap::const_iterator it = addressToPositionMap.find(address);
...
void PositionTableModified::setPosition(const L3Address& address, const Coord& coord)
{
...
void PositionTableModified::removeOldPositions(simtime_t timestamp)
{
...
void PositionTableModified::clear()
{
    addressToPositionMap.clear();
}

simtime_t PositionTableModified::getOldestPosition() const
{
